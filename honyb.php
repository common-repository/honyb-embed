 ...
 *
 * OR, to display as a span, with content, use a closing tag:
 *
 * [honyb sku="9781234567890"]This is some text[/honyb]
 *
 * @param $atts Array the attributes provided: sku, float, view
 * @param null $content Optional content to display. When present a span tag is used for inline links.
 * @return string
 */
function honyb_tag($atts, $content = null) {
    extract(shortcode_atts(array('sku' => '', 'float' => '', 'view' => 'product-brief', 'size' => 'small'), $atts));
    ...
    /** @var $float String */
    /** @var $view String */
    /** @var $size String small, product, large */
    return "<{$tag} class='honyb-embed {$float}' data-sku='{$sku}' data-view='{$view}' data-size='{$size}''>{$content}</{$tag}>";
}
